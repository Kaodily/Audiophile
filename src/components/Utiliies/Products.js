...
const Products = () => {
    return (
        <div className=' sm:flex md:justify-center sm:mt-16 sm:mx-8'>
            {datas.map(data => {
                return (
                    <div key={data.id} className='w-[100%] lg:w-[26%] lg:h-[240px] relative sm:mr-3 lg:mx-5  h-[150px] bg-white rounded-md my-[70px] shadow-sm '>
                         <div className='absolute top-[-30px] left-[50%] translate-x-[-50%]'>
<img src={data.image} alt={data.product} className='w-[130px] lg:w-[160px]  ' /> 
                              </div>
                        <h3 className='uppercase text-center text-[14px] pt-20 lg:pt-32 sm:text-[20px] font-bold  '>{data.product}</h3>    
