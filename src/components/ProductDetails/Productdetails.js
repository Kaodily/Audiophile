    ...
    <article className="pt-5 text-center lg:mx-[130px] ">
      {arr.map((data,index) => {
       let image =  window.innerWidth >= '800' ? data.image.mobile :  data.image.desktop
        return (
          <div key={data.id} className="py-9 lg:grid grid-cols-2 gap-5 lg:text-left">
            <div className="mx-[20px] rounded-md px-8 py-3 text-black ">
              <img src={image} alt={data.name} className='mx-auto'/>
            </div>
            <div className="lg:px-[80px] lg:pt-16"  style={index  % 2 === 0 ? {order: 1} : {order:-9999}}>
            <h3 className="uppercase mt-7 text-[12px] sm:text-[14px] lg:text-left tracking-widest font-bold text-[#D87D4A] text-center ">
              New product
            </h3>
            <h3 className="font-bold text-[20px] sm:text-[24px] sm:px-[200px] lg:px-[0] px-20 py-4 tracking-widest uppercase">
              {data.name}
            </h3>
